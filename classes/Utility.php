...
class Utility {
    ...
    {
        return wp_create_nonce(SCM_BLUEPRINT_PLUGIN_SLUG.'_nonce');
    }
    ...
    public static function adminBuildUrl($query_string)
    {
        ...
        $site_url = get_site_url();
        $url = rtrim($site_url,'/');
        $adminUrl = $url.'/wp-admin/admin.php?page='. SCM_BLUEPRINT_ROUTE_HANDLE .'&' .  $query_string;

        return $adminUrl;
    }

    /**
     * redirect to a given url on front end base and params, use in SCM_BLUEPRINT shortcode
     *
     * @param $query_string
     */
    public static function frontRedirectTo($query_string)
    {
        ...
        // build the whole url
        $actual_link = get_site_url() . $course_url . '?page='. SCM_BLUEPRINT_ROUTE_HANDLE .'&' .  $query_string;
    ...
    public static function templateBuffer($path, $data)
    {
        ob_start();
        $scmData = $data;
        $path = SCM_BLUEPRINT_PATH.'view/'.$path;
        include_once $path;
    ...
    public static function addFilterAdminOnly()
    {
        if( ! is_admin() )
        {
            self::setFlashMessage('Woops! Looks like something went wrong..If you believe this is a system error, please contact the administrator.','success');
            self::frontRedirectTo('state=Front&action=index');
            return;
        }
    }
    ...
    /**
     * helper to parse meta data into an object
     *
     * @param $meta
     * @return \stdClass
     */
    public static function parseMeta($meta)
    {
        ...
        {
            $meta->each(function($meta) use (&$Obj)
            {
    ...
    /**
     * helper to convert string to slug format
     *
     * @param $string
     * @return mixed
     */
