...
use SCM_BLUEPRINT\Controller\MainPageController as MainPageController;

class RouterException extends \Exception{};

class Router {

    const SCM_BLUEPRINTBaseRoute = SCM_BLUEPRINT_ROUTE_HANDLE;
    ...
    /**
     * the main SCM_BLUEPRINT route, whether to trigger all stuffs here or not
     *
     * @var
     */
    protected $baseRoute;
    ...
    public function boot()
    {
        ...
        if( $this->isBaseRouteIsSCM_BLUEPRINT() )
        {
    ...
    /**
     * check if the base route is SCM_BLUEPRINT Route, this is the foundation of routing
     * this will be the basis whether we will do stuffs in here or just totally ignore all
     *
     * @return bool
     */
    protected function isBaseRouteIsSCM_BLUEPRINT()
    {
        if($this->baseRoute == self::SCM_BLUEPRINTBaseRoute) return true;
    ...
    {
        return ucfirst(Utility::cleanText($_GET['state']));
    }
    ...
    {
        return Utility::cleanText($_GET['action']);
    }
    ...
    {
        return Utility::cleanText($_POST['postAction']);
    }
    ...
    {
        return Utility::cleanText( isset($_POST['_nonce']) ? $_POST['_nonce'] : '' );
    }
    ...
    protected function buildControllerClassName($state)
    {
        ...
        $controllerClass = "SCM_BLUEPRINT\\Controller\\{$state}Controller";
    ...
    {
        if( ! wp_verify_nonce($postNonce,SCM_BLUEPRINT_PLUGIN_SLUG.'_nonce') ) return false;
